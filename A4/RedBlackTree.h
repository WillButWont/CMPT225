 ...
 * Purpose: RedBlackTree class implementation
 */
...
class RedBlackTree
{
  ...
  private:
    ...
    // Helper function for tdisplay
    void tdisplay(RedBlackNode *t, int depth) const
    {
        if (t != nullNode)
        {
            tdisplay(t->left, depth + 1); // Traverse left subtree
            ...
            tdisplay(t->right, depth + 1); // Traverse right subtree
        }
    }

    // Helper function for countEven
    int counteven(RedBlackNode *t) const
    {
        if (t == nullNode)
            return 0;
